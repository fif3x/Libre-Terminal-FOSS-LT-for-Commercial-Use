...
#include <cstdlib>
#include <string>
#include "LibreTerminalHeaders/systmFOSS-LT.h"
#include "LibreTerminalHeaders/management.h"

bool safeMode = true; //set safeMode to true
...
void compileCPP(){
    ...
    system(compileCPPcommand.c_str()); //execute the command
};
void showName(){
...
void browseFiles(){
    std::cout << "Go to file (Enter Path): ";  //ask for the file path
...
void safeModeOFF(){
    safeMode = false; //set safeMode to false
};

void safeModeON(){
    safeMode = true; //set safeMode to true
};
...
void removeFile(){
    ...
    std::cout << "File to remove (Path/Name): "; //ask for the file path or name
